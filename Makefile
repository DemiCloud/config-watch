...
build: ## Development build → build/config-watch (debug symbols; version = "dev" if no tags)
	mkdir -p build
	go mod tidy
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o build/$(NAME) ./cmd/config-watch/
...
install: build ## Copy the built binary to $(DESTDIR)$(BINDIR) (default: /usr/local/bin)
	install -D -m 0755 build/$(NAME) $(DESTDIR)$(BINDIR)/$(NAME)
	@echo "Binary installed to $(DESTDIR)$(BINDIR)/$(NAME)."
	@echo "Run '$(DESTDIR)$(BINDIR)/$(NAME) install' from there to set up the systemd units."
